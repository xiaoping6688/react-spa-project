 ...
 * @param {Object} options 请求参数（必填）{headers, body, query, ...options}
...
const get = (url, args, options = {}) => {
  options.method = 'GET'
  options.query = args
  return callService(url, options)
}

const post = (url, args, options = {}) => {
  options.method = 'POST'
  options.body = args
  return callService(url, options)
}

const put = (url, args, options = {}) => {
  options.method = 'PUT'
  options.body = args
  return callService(url, options)
}

const patch = (url, args, options = {}) => {
  options.method = 'PATCH'
  options.body = args
  return callService(url, options)
}

const del = (url, args, options = {}) => {
